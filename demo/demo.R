...
netPlot <- eForce(n3)
newplot <- netPlot + option(size=c(1024,768), title="functionMap network output", showLabel = TRUE)
plot(newplot, Local = TRUE)

newplot <- netPlot + option(size=c(640,480), title="functionMap network output", showLabel = FALSE)
plot(newplot, Local = TRUE)
